...
void TestDosHeader()
{
    ...
    for (auto const& mod : modules)
    {
        ...
        auto const dos_header_raw = hadesmem::Read<IMAGE_DOS_HEADER>(
            process,
        ...
        cur_dos_header.SetBytesOnLastPage(
            cur_dos_header.GetBytesOnLastPage());
        ...
        cur_dos_header.SetNewHeaderOffset(
            cur_dos_header.GetNewHeaderOffset());

        auto const dos_header_raw_new = hadesmem::Read<IMAGE_DOS_HEADER>(
            process,
            cur_pe_file.GetBase());
        ...
        if (mod.GetHandle() != GetModuleHandle(L"ntdll"))
        {
            hadesmem::PeFile const pe_file_ntdll(
                ...
                hadesmem::PeFileType::Image);
            hadesmem::DosHeader const dos_header_ntdll(
                process, 
                pe_file_ntdll);
