...
void TestSectionList()
{
    ...
    for (auto const& mod : modules)
    {
        ...
        for (auto& section : sections)
        {
            ...
            section.SetPointerToRelocations(
                section.GetPointerToRelocations());
            section.SetPointerToLinenumbers(
                section.GetPointerToLinenumbers());
            ...
            if (mod.GetHandle() != GetModuleHandle(L"ntdll"))
            {
                hadesmem::PeFile const pe_file_ntdll(
                    ...
                    hadesmem::PeFileType::Image);
                hadesmem::Section const section_ntdll(
                    process, 
                    pe_file_ntdll, 
                    0);
