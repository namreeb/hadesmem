...
void TestNtHeaders()
{
    ...
    for (auto const& mod : modules)
    {
        ...
        auto const nt_headers_raw = hadesmem::Read<IMAGE_NT_HEADERS>(
            process,
        ...
        cur_nt_headers.SetNumberOfSections(
            cur_nt_headers.GetNumberOfSections());
        ...
        cur_nt_headers.SetNumberOfSymbols(
            cur_nt_headers.GetNumberOfSymbols());
        cur_nt_headers.SetSizeOfOptionalHeader(cur_nt_headers.
            GetSizeOfOptionalHeader());
        cur_nt_headers.SetCharacteristics(
            cur_nt_headers.GetCharacteristics());
        ...
        cur_nt_headers.SetSectionAlignment(
            cur_nt_headers.GetSectionAlignment());
        ...
        cur_nt_headers.SetMajorImageVersion(
            cur_nt_headers.GetMajorImageVersion());
        cur_nt_headers.SetMinorImageVersion(
            cur_nt_headers.GetMinorImageVersion());
        ...
        cur_nt_headers.SetWin32VersionValue(
            cur_nt_headers.GetWin32VersionValue());
        ...
        cur_nt_headers.SetSizeOfStackCommit(
            cur_nt_headers.GetSizeOfStackCommit());
        cur_nt_headers.SetSizeOfHeapReserve(
            cur_nt_headers.GetSizeOfHeapReserve());
        cur_nt_headers.SetSizeOfHeapCommit(
            cur_nt_headers.GetSizeOfHeapCommit());
        ...
        for (std::size_t i = 0; 
            i < cur_nt_headers.GetNumberOfRvaAndSizes(); 
            ++i)
        {
        ...
        auto const nt_headers_raw_new = hadesmem::Read<IMAGE_NT_HEADERS>(
            process,
            cur_nt_headers.GetBase());
