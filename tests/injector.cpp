...
void TestInjector()
{
    ...
    HMODULE const kernel32_mod_new = hadesmem::InjectDll(
        process,
        L"kernel32.dll", 
    ...
    SetLastError(0);
    auto const export_ret = CallExport(
        process, 
        kernel32_mod_new, 
        "GetCurrentProcessId");
    ...
    HMODULE const kernel32_mod_new_2 = hadesmem::InjectDll(
        process,
        L"kernel32.dll", 
        hadesmem::InjectFlags::kNone);
    ...
    if (GetModuleHandle(L"d3d9.dll") == nullptr)
    {
        ...
        BOOL const terminated = ::TerminateProcess(
            inject_data.GetProcess().GetHandle(),
            0);
