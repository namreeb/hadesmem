...
BOOST_AUTO_TEST_CASE(find_pattern_)
{
  ...
  // Ensure pattern result was saved back to parent
  BOOST_CHECK_EQUAL(zeros_pattern.GetAddress(), find_pattern[L"ZerosMinus1"]);
  // Ensure named map is the expected size
...
<HadesMem>
  ...
  BOOST_CHECK_THROW(find_pattern.Find(L"AA BB CC DD EE FF 11 22 33 44 55 66 "
    L"77 88 99 00 11 33 33 77", hadesmem::FindPatternFlags::kThrowOnUnmatch), 
    hadesmem::Error);
  
  // Check ScanData flag
  // Note: Pattern is for narrow string 'FindPattern' (without quotes)
  auto const find_pattern_str = find_pattern.Find(L"46 69 6E 64 50 61 74 74 "
