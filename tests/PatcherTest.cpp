...
// Detour manager
std::shared_ptr<HadesMem::PatchDetour> pDetour1;
...
BOOST_AUTO_TEST_CASE(PatcherTest)
{
  ...
  // Create byte patcher
  HadesMem::PatchRaw Patch1(MyMemory, TestMem1.GetBase(), Data1);
  // Get original memory contents
  auto const Orig1 = MyMemory.ReadList<std::vector<BYTE>>(
    TestMem1.GetBase(), 5);
  // Apply patch
  Patch1.Apply();
  // Get patched memory contents
  auto const Apply1 = MyMemory.ReadList<std::vector<BYTE>>(
    TestMem1.GetBase(), 5);
  // Remove patch
  Patch1.Remove();
  // Get unpatched memory contents
  auto const Remove1 = MyMemory.ReadList<std::vector<BYTE>>(
    TestMem1.GetBase(), 5);
  ...
  pDetour1.reset(new HadesMem::PatchDetour(MyMemory, 
