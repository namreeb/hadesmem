  ...
  std::vector<PVOID> const& args)
{
  ...
  return Call(process, addresses, call_convs, args_full)[0];
}

std::vector<RemoteFunctionRet> Call(Process const& process, 
  std::vector<LPCVOID> addresses, 
  std::vector<CallConv> call_convs, 
  std::vector<std::vector<PVOID>> const& args_full) 
{
  ...
  for (std::size_t i = 0; i < addresses.size(); ++i)
  {
    ...
    compiler.newFunction(CALL_CONV_COMPAT_STDCALL, BuildFunction1<int, int>());
    
    GPVar set_last_error_var(c.newGP());
    compiler.mov(set_last_error_var, AsmJit::imm(reinterpret_cast<DWORD_PTR>(set_last_error.GetBase()));
    ECall* ctx = c.call(set_last_error_var);
    ctx->setPrototype(CALL_CONV_COMPAT_STDCALL, FunctionBuilder3<void, int>());
