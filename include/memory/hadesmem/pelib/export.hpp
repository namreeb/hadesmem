...
namespace hadesmem
{
    ...
    public:
            ...
            forwarded_(false)
        {
            ...
            if (DWORD const num_names = export_dir.GetNumberOfNames())
            {
                ...
                if (name_ord_iter != std::end(name_ordinals))
                {
                    ...
                    DWORD const name_rva = Read<DWORD>(
                        ...
                        std::begin(name_ordinals), 
                        name_ord_iter));
                    name_ = ReadString<char>(
                        process, 
                        RvaToVa(process, pe_file, name_rva));
                }
            }
            ...
            {
                forwarded_ = true;
                forwarder_ = ReadString<char>(
                    process, 
                    RvaToVa(process, pe_file, func_rva));

                std::string::size_type const split_pos = 
                    forwarder_.rfind('.');
                if (split_pos != std::string::npos)
                {
                    forwarder_split_ = std::make_pair(
                        forwarder_.substr(0, split_pos),
