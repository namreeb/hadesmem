...
public:
  ...
  explicit RelocationIterator(Process const& process,
                              ...
                              PWORD start,
                              DWORD count)
    : impl_(std::make_shared<Impl>(process, pe_file, count))
  {
    try
    {
      if (!count)
      ...
      impl_->relocation_ = Relocation(process, pe_file, start);
    }
...
private:
  struct Impl
  {
    explicit Impl(Process const& process,
                  PeFile const& pe_file,
                  DWORD count) HADESMEM_DETAIL_NOEXCEPT : process_(&process),
                                                          pe_file_(&pe_file),
                                                          relocation_(),
                                                          count_(count),
                                                          cur_(0)
