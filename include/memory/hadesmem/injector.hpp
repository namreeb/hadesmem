...
namespace hadesmem
{
        ...
        std::uint32_t flags)
    {
        ...
        if (add_path && detail::IsPathRelative(path_real))
        {
            HADESMEM_DETAIL_THROW_EXCEPTION(Error() <<
                ErrorString("Cannot modify search order unless an absolute "
        ...
        std::size_t const path_buf_size = 
            (path_real.size() + 1) * sizeof(wchar_t);
        ...
        std::string const& export_name)
    {
        ...
        auto const export_ptr = FindProcedure(
            process, 
            module_remote, 
            export_name);
        ...
        std::uint32_t flags)
    {
        ...
        try
        {
            ...
            {
                if (::ResumeThread(thread_handle.GetHandle()) == 
                    static_cast<DWORD>(-1))
                {
                    DWORD const last_error = ::GetLastError();
                    HADESMEM_DETAIL_THROW_EXCEPTION(Error() <<
