...
public:
  explicit PatchDetour2(Process const& process,
    TargetFuncT target,
    DetourFuncT detour, void* context)
    : process_{ &process },
    target_{ target },
    detour_{ detour },
  ...
  explicit PatchDetour(Process&& process,
    TargetFuncT target,
    DetourFuncT detour, void* context) = delete;

  PatchDetour2(PatchDetour2 const& other) = delete;

  PatchDetour2& operator=(PatchDetour2 const& other) = delete;

  PatchDetour2(PatchDetour2&& other)
  ...
  PatchDetour2& operator=(PatchDetour2&& other)
  {
    RemoveUnchecked();
...
#error "[HadesMem] Unsupported architecture."
  ...
  detail::PatchContext<TargetFuncT> context_;
};

namespace detail
{

  template <typename R, typename... Args> class PatchContext < R(__cdecl*)(Args...) >
  {
  public:
    PatchContext(PatchDetour2<R(__cdecl*)(Args...)> const& patch, void* user_context)
      patch_(&patch), user_context_(user_context)
    {
    }

    static R Stub(Args...&& args)
    {
      auto const ctx = static_cast<PatchStub*>(winternl::GetCurrentTeb()->NtTib.ArbitraryUserPointer);
      return ctx->Stub(std::forward<Args>(args)...);
    }

  private:
    R StubImpl(Args...&& args)
    {
      return patch_->detour_(user_context_, std::forward<Args>(args)...);
    }

    PatchDetour2<R(__cdecl*)(Args...)> const* patch_;
    void* user_context_;
  };
}
