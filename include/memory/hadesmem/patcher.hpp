...
#error "[HadesMem] Unsupported architecture."
            ...
            HADESMEM_DETAIL_TRACE_A(detour_near 
                ? "Detour near." 
                : "Detour far.");
            DWORD_PTR const jump_size = detour_near 
                ? kJumpSize32 
                : kJumpSize64;
            ...
            do
            {
                ...
                std::size_t const sdword_size_bits = 
                    ...
                    && op->size == sdword_size_bits)
                {
                    ...
                    PVOID const jump_target = reinterpret_cast<PBYTE>(
                        static_cast<DWORD_PTR>(insn_base)) + 
                        insn_target + 
                        insn_len;
    ...
    private:
        ...
        std::unique_ptr<Allocator> AllocatePageNear(PVOID address)
        {
            ...
            for (LONG_PTR base = reinterpret_cast<LONG_PTR>(address), 
                index = 0;
                base + index < search_end 
                || base - index > search_beg;
                index += page_size)
            {
...
#error "[HadesMem] Unsupported architecture."
        ...
        DWORD_PTR WriteJump(PVOID address, PVOID target)
        {
            ...
            else
            {
                ...
                jit.jmp(AsmJit::qword_ptr(
                    label, 
                    static_cast<sysint_t>(disp)));
...
#error "[HadesMem] Unsupported architecture."
            ...
            {
                jump_buf.erase(
                    std::begin(jump_buf) + kJumpSize32, 
                    std::end(jump_buf));
        ...
        DWORD_PTR WriteCall(PVOID address, PVOID target)
        {
            ...
            jit.bind(label);
            LONG_PTR const disp = 
                reinterpret_cast<LONG_PTR>(tramp_addr)-
