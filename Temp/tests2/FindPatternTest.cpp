...
BOOST_AUTO_TEST_CASE(ConstructorsTest)
{
  ...
  // Create findpattern
  HadesMem::FindPattern MyFindPattern(MyMemory);
  
  // Test copying, assignement, and moving
  HadesMem::FindPattern MyOtherFindPattern(MyFindPattern);
  BOOST_CHECK(MyFindPattern == MyOtherFindPattern);
  MyFindPattern = MyOtherFindPattern;
  BOOST_CHECK(MyFindPattern == MyOtherFindPattern);
  HadesMem::FindPattern MovedFindPattern(std::move(MyOtherFindPattern));
  BOOST_CHECK(MovedFindPattern == MyFindPattern);
  HadesMem::FindPattern NewTestFindPattern(MyFindPattern);
  MyFindPattern = std::move(NewTestFindPattern);
  BOOST_CHECK(MyFindPattern == MovedFindPattern);
}

BOOST_AUTO_TEST_CASE(PatternTest)
{
  // Create memory manager for self
  ...
  HadesMem::FindPattern MyFindPattern(MyMemory, hSelf);
    
  // Create pattern scanner targetting self (using default constructor this 
  // time)
  MyFindPattern = HadesMem::FindPattern(MyMemory);
    
  // Ensure constructor throws if an invalid module handle is specified
  BOOST_CHECK_THROW(MyFindPattern = HadesMem::FindPattern(MyMemory, 
  ...
  auto const pNopRel = MyFindPattern.Find(L"90", 
    HadesMem::FindPattern::FindFlags_RelativeAddress);
  ...
  // a different name
  HadesMem::Pattern NopPattern(MyFindPattern, L"90", L"NopPlus1");
  // Apply 'Add' manipulator to pattern and save back to parent
  NopPattern << HadesMem::PatternManipulators::Add(1) << 
    HadesMem::PatternManipulators::Save();
  ...
  // Test stream-based pattern scanner by scanning for the same pattern, with 
  // a different name  
  HadesMem::Pattern ZerosPattern(MyFindPattern, L"00 ?? 00", 
    L"ZerosMinus1");
  // Apply 'Sub' manipulator to pattern and save back to parent
  ZerosPattern << HadesMem::PatternManipulators::Sub(1) << 
    HadesMem::PatternManipulators::Save();
  // Ensure manipulator was correctly applied
  ...
  // fail in that case.
  HadesMem::Pattern CallPattern(MyFindPattern, L"E8", 
    HadesMem::FindPattern::FindFlags_RelativeAddress);
  ...
  // relative call).
  CallPattern << HadesMem::PatternManipulators::Add(1) << 
    HadesMem::PatternManipulators::Rel(5, 1);
  // Ensure pattern was found
  ...
  BOOST_CHECK_THROW(MyFindPattern.Find(L"AA BB CC DD EE FF 11 22 33 44 55 66 "
    L"77 88 99 00 11 33 33 77", HadesMem::FindPattern::
    FindFlags_ThrowOnUnmatch), HadesMem::FindPattern::Error);
  ...
  auto const pFindPatternStr = MyFindPattern.Find(L"46 69 6E 64 50 61 74 74 "
    L"65 72 6E", HadesMem::FindPattern::FindFlags_ScanData);
