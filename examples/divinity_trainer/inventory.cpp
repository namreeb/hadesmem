...
void DumpInventory(hadesmem::Process const& process, Inventory* inventory_ptr)
{
  ...
  {
    auto const item_handles = hadesmem::ReadVector<std::uint32_t>(
      process, inventory.item_handles_, inventory.capacity_);
    for (std::uint32_t i = 0;
         i < static_cast<std::uint32_t>(item_handles.size());
         ++i)
    {
      printf("Item %u: %08X\n", i, item_handles[i]);
    }
