...
public:
    ...
    SystemProcessInformationEnum(SystemProcessInformationEnum const&) 
        = delete;

    SystemProcessInformationEnum& operator=(
        SystemProcessInformationEnum const&) = delete;
    ...
    void Advance() HADESMEM_DETAIL_NOEXCEPT
    {
        ...
        if (buffer_->NextEntryOffset)
        {
            buffer_ = reinterpret_cast<winternl::SYSTEM_PROCESS_INFORMATION*>(
                reinterpret_cast<DWORD_PTR>(buffer_)+
                buffer_->NextEntryOffset);
        }
        else
        {
            buffer_ = nullptr;
    ...
    std::wstring GetName() const
    {
        ...
        auto const str_end = std::find(
            buffer_->ImageName.Buffer,
...
private:
    winternl::SYSTEM_PROCESS_INFORMATION* GetRealBuffer(
        winternl::SYSTEM_INFORMATION_CLASS info_class,
        void* buffer) const HADESMEM_DETAIL_NOEXCEPT
    {
        ...
        else if (info_class == winternl::SystemSessionProcessInformation)
        {
            return static_cast<winternl::SYSTEM_PROCESS_INFORMATION*>(
                static_cast<winternl::SYSTEM_SESSION_PROCESS_INFORMATION*>(
                buffer)->Buffer);
        }
        else
        {
    ...
    PULONG return_length) HADESMEM_DETAIL_NOEXCEPT
{
    ...
    auto const ret = nt_query_system_information(
        system_information_class,
        system_information,
        system_information_length,
        return_length);
    ...
    if (system_information_class != 
        winternl::SystemProcessInformation &&
        system_information_class != 
        winternl::SystemExtendedProcessInformation &&
        system_information_class != 
        winternl::SystemSessionProcessInformation && 
        system_information_class != 
        winternl::SystemFullProcessInformation)
    {
...
extern "C" HADESMEM_DETAIL_DLLEXPORT DWORD_PTR Load() HADESMEM_DETAIL_NOEXCEPT
{
    try
    {
        ...
        g_nt_query_system_information_hk.reset(new hadesmem::PatchDetour(
            *g_process,
