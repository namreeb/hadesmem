...
namespace
{
    ...
    void DumpRegions(hadesmem::Process const& process)
    {
        ...
        for (auto const& region : regions)
        {
            ...
            std::wcout << "\tSize: " << std::hex << region.GetSize() 
                << std::dec << "\n";
            std::wcout << "\tState: " << std::hex << region.GetState() 
                << std::dec << "\n";
            std::wcout << "\tProtect: " << std::hex << region.GetProtect()
                << std::dec << "\n";
            std::wcout << "\tType: " << std::hex << region.GetType() 
                << std::dec << "\n";
        }
    }
        ...
        hadesmem::PeFile const& pe_file)
    {
        ...
        std::wcout << "\n";
        std::wcout << "\t\tMagic: " << std::hex << dos_hdr.GetMagic() 
            << std::dec << "\n";
        std::wcout << "\t\tBytesOnLastPage: " << std::hex
            << dos_hdr.GetBytesOnLastPage() << std::dec << "\n";
        std::wcout << "\t\tPagesInFile: " << std::hex 
            << dos_hdr.GetPagesInFile() << std::dec << "\n";
        std::wcout << "\t\tRelocations: " << std::hex 
            << dos_hdr.GetRelocations() << std::dec << "\n";
        ...
        std::wcout << "\t\tOverlayNum: " << std::hex 
            << dos_hdr.GetOverlayNum() << std::dec << "\n";
        ...
        std::wcout << std::dec << "\n";
        std::wcout << "\t\tOEMID: " << std::hex << dos_hdr.GetOEMID() 
            << std::dec << "\n";
        ...
        hadesmem::PeFile const& pe_file)
    {
        ...
        std::wcout << "\t\tMagic: " << std::hex << nt_hdrs.GetMagic() 
            << std::dec << "\n";
        std::wcout << "\t\tMajorLinkerVersion: " 
            << nt_hdrs.GetMajorLinkerVersion() << "\n";
        std::wcout << "\t\tMinorLinkerVersion: " 
            << nt_hdrs.GetMinorLinkerVersion() << "\n";
        std::wcout << "\t\tSizeOfCode: " << std::hex 
            << nt_hdrs.GetSizeOfCode() << std::dec << "\n";
        ...
        std::wcout << "\t\tBaseOfCode: " << std::hex 
            << nt_hdrs.GetBaseOfCode() << std::dec << "\n";
#if defined(HADESMEM_DETAIL_ARCH_X86) 
        std::wcout << "\t\tBaseOfData: " << std::hex 
            << nt_hdrs.GetBaseOfData() << std::dec << "\n";
#endif
        std::wcout << "\t\tImageBase: " << std::hex 
            << nt_hdrs.GetImageBase() << std::dec << "\n";
        ...
        std::wcout << "\t\tMajorImageVersion: " 
            << nt_hdrs.GetMajorImageVersion() << "\n";
        std::wcout << "\t\tMinorImageVersion: " 
            << nt_hdrs.GetMinorImageVersion() << "\n";
        ...
        std::wcout << "\t\tWin32VersionValue: " 
            << nt_hdrs.GetWin32VersionValue() << "\n";
        std::wcout << "\t\tSizeOfImage: " << std::hex 
            << nt_hdrs.GetSizeOfImage() << std::dec << "\n";
        std::wcout << "\t\tSizeOfHeaders: " << std::hex
            << nt_hdrs.GetSizeOfHeaders() << std::dec << "\n";
        std::wcout << "\t\tCheckSum: " << std::hex 
            << nt_hdrs.GetCheckSum() << std::dec << "\n";
        std::wcout << "\t\tSubsystem: " << std::hex 
            << nt_hdrs.GetSubsystem() << std::dec << "\n";
        ...
        std::wcout << "\t\tLoaderFlags: " << std::hex 
            << nt_hdrs.GetLoaderFlags() << std::dec << "\n";
        ...
        hadesmem::PeFile const& pe_file)
    {
        ...
        for (auto const& s : sections)
        {
            ...
            std::wcout << "\t\tVirtualAddress: " << std::hex 
                << s.GetVirtualAddress() << std::dec << "\n";
            std::wcout << "\t\tVirtualSize: " << std::hex 
                << s.GetVirtualSize() << std::dec << "\n";
            std::wcout << "\t\tPointerToRawData: " << std::hex
                << s.GetPointerToRawData() << std::dec << "\n";
            std::wcout << "\t\tSizeOfRawData: " << std::hex 
                << s.GetSizeOfRawData() << std::dec << "\n";
        ...
        for (auto const& e : exports)
        {
            ...
            if (e.IsForwarded())
            {
                std::wcout << "\t\tForwarder: " << e.GetForwarder().c_str() 
                    << "\n";
                std::wcout << "\t\tForwarderModule: " 
                    << e.GetForwarderModule().c_str() << "\n";
                std::wcout << "\t\tForwarderFunction: " <<
                    e.GetForwarderFunction().c_str() << "\n";
                std::wcout << "\t\tIsForwardedByOrdinal: " 
                    << e.IsForwardedByOrdinal() << "\n";
                if (e.IsForwardedByOrdinal())
                {
                    try
                    {
                        std::wcout << "\t\tForwarderOrdinal: " 
        ...
        hadesmem::PeFile const& pe_file)
    {
        ...
        for (auto const& dir : import_dirs)
        {
            ...
            std::wcout << "\t\tName (Raw): " << std::hex 
                << dir.GetNameRaw() << std::dec << "\n";
            std::wcout << "\t\tName: " << dir.GetName().c_str() << "\n";
            std::wcout << "\t\tFirstThunk: " << std::hex 
                << dir.GetFirstThunk() << std::dec << "\n";
            ...
            for (auto const& thunk : import_thunks)
            {
                ...
                std::wcout << "\t\t\tFunction: " << std::hex 
                    << thunk.GetFunction() << std::dec << "\n";
            }
    ...
    void DumpModules(hadesmem::Process const& process)
    {
        ...
        for (auto const& module : modules)
        {
            ...
            std::wcout << "\tHandle: " << PtrToString(module.GetHandle()) 
                << "\n";
            std::wcout << "\tSize: " << std::hex << module.GetSize() 
                << std::dec << "\n";
    ...
    void DumpProcessEntry(hadesmem::ProcessEntry const& process_entry)
    {
        ...
        std::wcout << "WoW64: " << (hadesmem::IsWoW64(*process) 
            ? "Yes" : "No") << "\n";
...
int main(int /*argc*/, char* /*argv*/[])
{
    try
    {
        ...
        opts_desc.add_options()
            ...
            ("pid", boost::program_options::value<DWORD>(), 
            "target process id")
            ("file", boost::program_options::wvalue<std::wstring>(), 
            "target file")
            ("dir", boost::program_options::wvalue<std::wstring>(), 
            "target dir")
            ;
        ...
        if (var_map.count("pid"))
        {
            ...
            {
                HADESMEM_DETAIL_THROW_EXCEPTION(hadesmem::Error() <<
                    hadesmem::ErrorString("Failed to find requested "
                    "process."));
            }
        }
