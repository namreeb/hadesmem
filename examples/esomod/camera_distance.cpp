...
void SetMaxCameraDistance(hadesmem::Process const& process, float value)
{
  ...
  std::cout << "Got camera manager. [" << static_cast<void*>(global_pointer_manager)
            << "].\n";

  // eso.live.1.1.3.998958 (dumped with module base of 002D0000)
  // .text:0043C6D0                 mov     eax, [ecx + 4Ch]
  auto const kCameraConstraintsOffset = 0x4C;
  auto const camera_constraints =
    hadesmem::Read<std::uint8_t*>(process, global_pointer_manager + kCameraConstraintsOffset);
  std::cout << "Got camera constraints. [" << static_cast<void*>(camera_constraints) << "].\n";

  // eso.live.1.1.3.998958 (dumped with module base of 002D0000)
  // .text:0033AD65                 fld     dword ptr[esi + 64h]
  auto const kMaxCameraDistanceOffset = 0x64;
  auto const max_camera_distance = camera_constraints + kMaxCameraDistanceOffset;
  std::cout << "Writing max camera distance. [" << static_cast<void*>(max_camera_distance)
            << "].\n";
