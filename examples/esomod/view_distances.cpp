...
void SetViewDistances(hadesmem::Process const& process, float* min, float* max)
{
  ...
  auto const max_view_distance_ref = static_cast<std::uint8_t*>(
  ...
  if (min)
  {
    auto const old_distance = hadesmem::Read<float>(process, min_view_distance_ptr);
  ...
  if (max)
  {
    auto const old_distance = hadesmem::Read<float>(process, max_view_distance_ptr);
