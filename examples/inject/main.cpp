...
namespace
{
    ...
    boost::program_options::options_description GetOptionsDesc()
    {
        ...
        opts_desc.add_options()
            ...
            ("module", boost::program_options::wvalue<std::wstring>(), 
            "module path")
            ("path-resolution", "perform (local) path resolution on "
            "module path")
        ...
        boost::program_options::options_description const& opts_desc)
    {
        ...
        boost::program_options::store(boost::program_options::
            wcommand_line_parser(args).options(opts_desc).run(), 
            var_map);
...
int main(int argc, char* /*argv*/[])
{
    try
    {
        ...
        auto const module_path = GetOptionValue<std::wstring>(
            "module", 
            var_map);
        bool const path_resolution = IsOptionSet(
            "path-resolution", 
            var_map);
        bool const add_path = IsOptionSet(
            "add-path", 
            var_map);
        ...
        if (has_pid || has_name)
        {
            ...
            else
            {
                ...
                if (path_resolution && hadesmem::detail::IsPathRelative(
                    path_real))
                {
        ...
        else
        {
            ...
            std::wcout << "Process ID: " << inject_data.GetProcess() 
                << ".\n";
            std::wcout << "Module Base: " << PtrToString(
                inject_data.GetModule()) << ".\n";
            std::wcout << "Export Return: " << inject_data.GetExportRet() 
                << ".\n";
            std::wcout << "Export LastError: " 
                << inject_data.GetExportLastError() << ".\n";
        }
