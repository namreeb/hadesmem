...
void DumpItem(divinity::Item* item, int debug_id)
{
  ...
  HADESMEM_DETAIL_TRACE_FORMAT_A("CurrentTemplate: %p",
                                 item->item_data_.current_template_);
  auto const current_item_template = item->item_data_.current_template_;
  (void)current_item_template;
  HADESMEM_DETAIL_TRACE_FORMAT_A("CurrentTemplate::UUID: %s",
                                 current_item_template->uuid_);
  HADESMEM_DETAIL_TRACE_FORMAT_A(
    "CurrentTemplate::Name: %s",
    GetStdStringRaw(&current_item_template->name_));
  HADESMEM_DETAIL_TRACE_FORMAT_A("OriginalTemplate: %p",
                                 item->item_data_.original_template_);
  auto const original_item_template = item->item_data_.original_template_;
  (void)original_item_template;
  HADESMEM_DETAIL_TRACE_FORMAT_A("OriginalTemplate::UUID: %s",
                                 original_item_template->uuid_);
  HADESMEM_DETAIL_TRACE_FORMAT_A(
    "OriginalTemplate::Name: %s",
    GetStdStringRaw(&original_item_template->name_));
  ...
  if (item->item_data_.stats_)
  {
    ...
    HADESMEM_DETAIL_TRACE_FORMAT_A("Stats::IsIdentified: %d",
                                   item_stats->is_identified_);
    HADESMEM_DETAIL_TRACE_FORMAT_A("Stats::Durability: %d",
                                   item_stats->durability_);
    HADESMEM_DETAIL_TRACE_FORMAT_A("Stats::DurabilityCounter: %d",
    ...
    HADESMEM_DETAIL_TRACE_FORMAT_A("Stats::ItemType: %s",
                                   item_stats->item_type_);
  }
  HADESMEM_DETAIL_TRACE_FORMAT_A("Inventory: %08X",
                                 item->item_data_.inventory_);
  ...
  HADESMEM_DETAIL_TRACE_FORMAT_A("SurfaceCheckTimer: %f",
                                 item->item_data_.surface_check_timer_);
  HADESMEM_DETAIL_TRACE_FORMAT_A("LifeTime: %f", item->item_data_.life_time_);
  HADESMEM_DETAIL_TRACE_FORMAT_A("ItemMachine: %p",
                                 item->item_data_.item_machine_);
  HADESMEM_DETAIL_TRACE_FORMAT_A("PlanManager: %p",
                                 item->item_data_.plan_manager_);
  HADESMEM_DETAIL_TRACE_FORMAT_A("VariableManager: %p",
                                 item->item_data_.variable_manager_);
